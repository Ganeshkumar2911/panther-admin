<template>
  <div class="min-h-screen bg-background text-primary-text flex flex-col -m-4 lg:-m-6">
    ...
    <div class="border-b border-primary-border px-6 py-4">
      <div class="flex items-start gap-4 mb-6">
        ...
        <!-- Avatar -->
        <div class="w-12 h-12 rounded-full bg-accent flex items-center justify-center text-white font-semibold text-lg shrink-0">
          {{ initials }}
        </div>
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center gap-3 flex-wrap">
            <h1 class="text-lg font-semibold text-primary-text">{{ user.name }}</h1>
            <span
              class="px-2 py-0.5 rounded-full text-[11px] font-medium"
              :class="user.is_active ? 'bg-primary-green/10 text-primary-green' : 'bg-primary-red/10 text-primary-red'"
            >
              {{ user.is_active ? 'Active' : 'Inactive' }}
            </span>
            <span class="px-2 py-0.5 rounded-full text-[11px] font-medium bg-primary-blue/10 text-primary-blue">
              {{ user.state }}
            </span>
            <span class="text-[11px] text-secondary-text uppercase tracking-widest">
              {{ user.role }}
            </span>
          </div>
          ...
          <div class="flex items-center gap-6 mt-1 flex-wrap">
            <span class="text-[11px] text-secondary-text">
              CLIENT ID <span class="text-primary-text font-medium tabular-nums">#{{ user.id }}</span>
            </span>
            <span class="text-[11px] text-secondary-text">
              WHITELABEL <span class="text-primary-text font-medium">{{ whitelabel }}</span>
            </span>
          </div>
        </div>
        ...
        <div class="hidden md:flex items-center gap-8 text-sm">
          <div>
            <p class="text-[11px] text-secondary-text uppercase tracking-widest mb-0.5">Email</p>
            <p class="text-primary-text tabular-nums">{{ user.email }}</p>
          </div>
          <div>
            <p class="text-[11px] text-secondary-text uppercase tracking-widest mb-0.5">Phone</p>
            <p class="text-primary-text tabular-nums">{{ user.phone_number }}</p>
          </div>
          <div>
            <p class="text-[11px] text-secondary-text uppercase tracking-widest mb-0.5">KYC</p>
            <span
              class="px-2 py-0.5 rounded-full text-[11px] font-medium"
              :class="kycClass"
            >
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <!-- LEFT SIDEBAR -->
      <aside class="w-52 shrink-0 border-r border-primary-border overflow-y-auto">
        <div class="p-3 space-y-1">
          <template v-for="section in sidebarSections" :key="section.key">
            ...
            <!-- Child routes -->
            <div v-else-if="!section.isInfo && expanded[section.key]" class="ml-3 pl-3 border-l border-primary-border space-y-0.5">
              <RouterLink
...
import {
  ...
  Phone, Mail, FileText, FileCheck, MessageSquare
...
const sidebarSections = computed(() => [
  {
    ...
    fields: [
      ...
      { label: 'Client ID', value: () => user.value.id ? `#${user.value.id}` : null },
    ],
  },
  {
    key: 'info',
    ...
    fields: [
      ...
      { label: 'Verification Channel', value: () => user.value.verification_channel },
      { label: 'Docs Uploaded',        value: () => user.value.docs_uploaded },
      { label: 'Doc Approved',         value: () => user.value.doc_approved },
      { label: 'Sumsub Applicant ID',  value: () => user.value.sumsub_applicant_id },
    ],
  },
  {
    key: 'address',
  ...
  },
  {
    ...
    fields: [
      ...
      { label: 'Account Numbers',  value: () => user.value.account_numbers?.join(', ') },
      { label: 'Account Types',    value: () => user.value.account_types?.join(', ') },
      { label: 'Total Accounts',   value: () => user.value.total_accounts?.toString() },
      { label: 'Is IB',            value: () => user.value.is_ib != null ? (user.value.is_ib ? 'Yes' : 'No') : null },
