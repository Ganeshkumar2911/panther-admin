<template>
  <div class="px-4">
    ...
    <div v-if="activeTab === 'transactions'">
        ...
        >
          ...
          <Tooltip text="Export" position="left">
            <button
              ...
              class="cursor-pointer rounded-lg p-2 text-secondary-text hover:bg-background hover:text-primary-text transition-colors border border-primary-border disabled:opacity-60 disabled:cursor-not-allowed"
              @click="store.exportLedger"
            >
      ...
      <!-- Table -->
      <div
        class="w-full border border-primary-border rounded-xl overflow-x-auto bg-card-background"
      >
          ...
          <tbody v-if="store.loading">
            ...
            >
              ...
              <td class="p-3 flex justify-end">
                <div class="h-3.5 w-20 bg-background rounded" />
              </td>
            </tr>
          </tbody>
          ...
          <tbody v-else>
            ...
            >
              ...
              <td class="p-3">
                ...
                <p
                  v-if="entry.txid"
                  class="text-[10px] text-secondary-text font-mono truncate max-w-[150px]"
                  :title="entry.txid"
                >
                  TXID: {{ entry.txid }}
                </p>
                <p
                  v-else-if="entry.external_payment_id"
                  class="text-[10px] text-secondary-text truncate max-w-[150px]"
                >
                  Ext: {{ entry.external_payment_id }}
                </p>
              </td>
              ...
              <td class="p-3">
                <span
                  ...
                  :class="amountClass(entry.type, entry.amount)"
                >
                  {{ entry.type === "deposit" ? "+" : "-"
                  }}{{ formatMoney(entry.amount, entry.broker_currency) }}
                </span>
              </td>
              ...
              <td class="p-3">
                <span
                  class="text-[11px] font-medium px-2 py-0.5 rounded-full border capitalize"
                  :class="
                    statusClass(entry.payment_status ?? entry.approval_status)
                  "
                >
              ...
              <!-- Date -->
              <td
                class="p-3 text-xs text-secondary-text text-right whitespace-nowrap"
              >
...
import { BookOpen, FolderUp, RefreshCw } from "lucide-vue-next";
