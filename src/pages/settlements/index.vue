      ...
      <template v-else>
        ...
        >
          ...
          <p class="text-2xl font-medium text-primary-text">
            ${{ formatNum(store.summary.total_fees) }}
          </p>
        </div>
        <div
          class="bg-card-background border border-primary-border rounded-xl p-4"
        >
          ...
          <p class="text-2xl font-medium text-green-700">
            ${{ formatNum(store.summary.broker_net) }}
          </p>
        </div>
        <div
          class="bg-card-background border border-primary-border rounded-xl p-4"
        >
          ...
          <p class="text-2xl font-medium text-primary-text">
            ${{ formatNum(store.summary.ib_pool) }}
          </p>
        </div>
      </template>
    </div>
      ...
      <table class="w-full border-collapse">
        ...
        <tbody v-if="store.loading">
          ...
          >
            ...
            <td class="p-3 flex justify-end">
              <div class="h-5 w-14 bg-card-background rounded" />
            </td>
          </tr>
        </tbody>
          ...
          >
            ...
            <td class="p-3">
              <span
                ...
                :class="row.net_profit >= 0 ? 'text-green-700' : 'text-red-700'"
              >
                {{ row.net_profit >= 0 ? "+" : "" }}${{
                  formatNum(row.net_profit)
                }}
              </span>
            </td>

            <td class="p-3 text-xs text-primary-text tabular-nums">
              ${{ formatNum(row.gross_fee) }}
            </td>

            <td class="p-3 text-xs text-primary-text tabular-nums">
              ${{ formatNum(row.fm_share) }}
            </td>

            <td class="p-3 text-xs text-primary-text tabular-nums">
              ${{ formatNum(row.ib_pool) }}
            </td>

            <td class="p-3 text-xs text-primary-text tabular-nums">
              ${{ formatNum(row.broker_net) }}
            </td>
...
import { FolderUp, ReceiptText } from "lucide-vue-next";
