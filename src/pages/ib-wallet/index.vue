  ...
  <div class="px-4">
      ...
      <template v-else>
        ...
        >
          ...
          <p class="text-2xl font-medium text-green-700">
            +${{ formatNum(store.summary.total_affiliate_commission) }}
          </p>
        </div>
        <div
          class="bg-card-background border border-primary-border rounded-xl p-4"
        ...
        >
          ...
          <p class="text-2xl font-medium text-primary-text">
            ${{ formatNum(store.summary.total_ib_fee) }}
          </p>
        </div>
    ...
    >
      ...
      >
        ...
        <Tooltip text="Export">
          <button
            class="p-2 rounded-lg border border-primary-border text-secondary-text hover:text-primary-text hover:bg-background transition-colors cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
            @click="store.exportIbWallet"
      ...
      <table class="w-full border-collapse">
        ...
        <tbody v-else>
          ...
          >
            ...
            <td class="p-3 text-xs text-secondary-text tabular-nums">
              ${{ formatNum(entry.balance_before) }}
            </td>
            <td class="p-3 text-xs text-primary-text tabular-nums">
              ${{ formatNum(entry.balance_after) }}
            </td>
            ...
            <td class="p-3 text-xs text-secondary-text max-w-[160px] truncate">
              {{ parseDescription(entry.description).text }}
            </td>

            <td class="p-3 text-xs text-secondary-text text-right">
              {{
                parseDescription(entry.description).date
                  ? formatDate(parseDescription(entry.description).date)
                  : "_"
              }}
            </td>
            <td class="p-3 text-xs text-secondary-text text-right">
...
import { Wallet, RotateCw, Plus, FolderUp } from "lucide-vue-next";
