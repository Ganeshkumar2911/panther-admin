<template>
  <div
    class="px-4 mx-auto max-w-7xl h-[calc(100vh-115px)] flex flex-col overflow-hidden"
  >
    <!-- Initial Loading State (Centered spinner overlay instead of skeleton layout wipe) -->
    <div
      v-if="store.detailLoading && !store.detail"
    ...
    <template v-else-if="store.detail">
      ...
      <div class="shrink-0 mb-4 space-y-1.5">
        <div class="flex flex-wrap items-center gap-2">
          <span
            class="text-xs font-semibold px-2.5 py-1 rounded-md bg-background border border-primary-border text-secondary-text font-mono"
          >
          ...
          <div
            ...
            :class="
              getTat(store.detail).isBreached
        ...
        <div class="flex items-center gap-3 text-xs text-secondary-text">
          <span
            >Created by
            <strong class="text-primary-text font-medium">{{
      ...
      <div class="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-3 gap-6 pb-2">
        ...
        <div
          class="lg:col-span-1 flex flex-col gap-4 overflow-y-auto no-scrollbar pr-1"
        >
          <!-- Assignee & Ticket Meta Card -->
          <div
            class="bg-card-background border border-primary-border rounded-2xl p-5 shadow-xs space-y-4 shrink-0"
          >
            <div
              class="flex items-center justify-between border-b border-primary-border pb-2.5"
            >
              <h2
            ...
            <div
              class="flex items-center gap-2 pb-1 border-b border-primary-border/60"
            >
              <button
                type="button"
                @click="refreshTicket"
                :disabled="store.detailLoading"
                class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border border-primary-border bg-card-background hover:bg-background text-secondary-text hover:text-primary-text text-xs font-medium transition-all cursor-pointer shadow-xs disabled:opacity-50"
                title="Refresh Ticket Details"
              >
                <RotateCw
                  class="w-3.5 h-3.5"
                  :class="{ 'animate-spin': store.detailLoading }"
                />
                <span>Refresh</span>
              </button>
              ...
              <button
                ...
                "
                @click="openStatusDialog"
                class="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-primary hover:bg-primary-hover text-btn-text-primary text-xs font-medium transition-all cursor-pointer shadow-xs"
              >
            ...
            <div>
              <div class="flex items-center justify-between mb-1">
                ...
                <button
                  v-if="hasPermission(['ticket.platform_update', 'ticket.update', 'ticket.assign']) && store.detail.assigned_staff && !isEditingStaff"
                  type="button"
              ...
              <div
                v-if="store.detail.assigned_staff && !isEditingStaff"
                class="flex items-center gap-2.5 p-2.5 rounded-lg bg-background border border-primary-border"
              >
                <div
                  class="w-8 h-8 rounded-full bg-primary/10 border border-primary/20 flex items-center justify-center text-xs font-semibold text-primary"
                >
                  {{
                    (store.detail.assigned_staff.name || "S")
                      .charAt(0)
                      .toUpperCase()
                  }}
                </div>
                ...
                <span
                  v-if="store.detail.assigned_role"
                  class="text-[10px] uppercase font-bold px-2 py-0.5 rounded-md bg-primary/10 text-primary border border-primary/20 shrink-0"
                >
              ...
              <div
                v-else-if="hasPermission(['ticket.platform_update', 'ticket.update', 'ticket.assign'])"
              ...
              <p
                v-else
                class="text-xs text-secondary-text italic p-2 bg-background rounded-lg border border-primary-border"
              >
            ...
            <div>
              ...
              >
                <div
                  class="w-8 h-8 rounded-full bg-accent/10 border border-accent/20 flex items-center justify-center text-xs font-semibold text-accent"
                >
                  {{
                    (store.detail.created_by_name || "C")
                      .charAt(0)
                      .toUpperCase()
                  }}
                </div>
                ...
                <span
                  class="ml-auto text-[10px] uppercase font-bold px-2 py-0.5 rounded-md bg-background text-secondary-text border border-primary-border"
                >
          ...
          <!-- Description Card -->
          <div
            class="bg-card-background border border-primary-border rounded-2xl p-5 shadow-xs shrink-0"
          >
            <h2
              class="text-xs font-bold uppercase tracking-wider text-secondary-text border-b border-primary-border pb-2.5 mb-3"
            >
              Initial Issue Description
            </h2>
            <p
              class="text-xs text-primary-text leading-relaxed whitespace-pre-line"
            >
        ...
        <div
          class="lg:col-span-2 flex flex-col bg-card-background border border-primary-border rounded-2xl shadow-xs overflow-hidden h-full min-h-0"
        >
          <!-- Chat Header (shrink-0) -->
          <div
            class="px-6 py-4 border-b border-primary-border bg-card-background flex items-center justify-between shrink-0"
          >
            <div class="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p class="text-[11px] text-secondary-text">
                  Chat history & resolution updates
                </p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-2">
              <span
                class="text-xs px-2.5 py-1 rounded-full bg-background border border-primary-border text-secondary-text font-medium"
              >
                {{ totalHistoryCount }}
                {{ totalHistoryCount === 1 ? "event" : "events" }}
              </span>
            </div>
          </div>
          ...
          >
            <div
              v-if="chatStream.length === 0"
              class="flex flex-col items-center justify-center h-full text-center p-6"
            >
              <div
                class="w-12 h-12 rounded-2xl bg-card-background border border-primary-border flex items-center justify-center text-secondary-text mb-3 shadow-xs"
              >
              ...
              <p class="text-xs text-secondary-text max-w-xs mt-1">
                Start chatting below to respond or add attachments to this
                ticket.
              </p>
            </div>
            ...
            <template v-else>
              ...
              >
                ...
                <div class="flex items-center justify-center my-4">
                  <span
                    class="text-[10px] font-semibold text-secondary-text bg-card-background border border-primary-border px-3 py-1 rounded-full uppercase tracking-wider shadow-2xs"
                  >
                    {{ group.dateLabel }}
                  </span>
                </div>
                ...
                <div v-for="item in group.messages" :key="item.id">
                  ...
                  >
                    <div
                      class="flex items-center gap-1.5 text-xs text-secondary-text bg-card-background/80 border border-primary-border/70 px-3 py-1.5 rounded-full shadow-2xs"
                    >
                      <UserCheck
                        v-if="item.event_type === 'staff_assigned'"
                        class="w-3.5 h-3.5 text-primary"
                      />
                      <CheckCircle2
                        v-else-if="item.event_type === 'status_changed'"
                        class="w-3.5 h-3.5 text-primary-green"
                      />
                      <AlertCircle
                        v-else
                        class="w-3.5 h-3.5 text-primary-yellow"
                      />
                      <span>{{ getSystemEventText(item) }}</span>
                      <span class="text-[10px] text-secondary-text/60 ml-1"
                  ...
                  >
                    ...
                    >
                      ...
                      <div class="flex items-center gap-2 mb-1 px-1">
                        <span
                          class="text-[11px] font-semibold text-primary-text"
                        >
                        ...
                        <span
                          v-if="item.isSending"
                          class="text-[10px] text-secondary-text flex items-center gap-1"
                        >
                          <Loader2
                            class="w-2.5 h-2.5 animate-spin text-primary"
                          />
                          <span>Sending...</span>
                        </span>
                          ...
                          :class="
                            item.comment_text
                              ? item.isMe
                                ...
                                : 'border-t border-primary-border'
                              : ''
                          "
                        >
                          <!-- Image Attachment -->
                          <div
                            v-if="isImageFile(item.file_url || item.file_name)"
                            class="space-y-1.5"
                          >
                            <a
                              :href="item.file_url"
                              ...
                              class="block overflow-hidden rounded-xl border max-w-sm hover:opacity-95 transition-opacity"
                              :class="
                                item.isMe
                            ...
                            >
                              <span class="truncate max-w-[200px]">{{
                                item.file_name || "Image attachment"
                              }}</span>
                              <a
                                :href="item.file_url"
                                target="_blank"
                            ...
                            <div
                              ...
                              :class="
                                item.isMe
                                  ...
                                  : 'bg-card-background border border-primary-border text-primary'
                              "
                            >
          ...
          <div
            v-if="selectedFile"
            class="px-4 py-2 bg-background border-t border-primary-border flex items-center justify-between"
          >
            <div class="flex items-center gap-2.5 min-w-0">
              ...
              <div class="min-w-0 text-xs">
                <p class="font-medium text-primary-text truncate">
                  {{ selectedFile.name }}
                </p>
...
const isSystemEvent = (item) => {
  const systemTypes = [
    ...
    "status_changed",
    "priority_changed",
    "ticket_created_system",
  ];
  return (
    systemTypes.includes(item.event_type) ||
    (!item.comment_text && !item.file_url)
  );
};
...
const chatStream = computed(() => {
  ...
  if (Array.isArray(store.detail.history) && store.detail.history.length > 0) {
    store.detail.history.forEach((h) => {
      ...
      const isMe =
        currentUserId.value && String(actorId) === String(currentUserId.value);
      rawItems.push({
    ...
    if (Array.isArray(store.detail.comments)) {
      store.detail.comments.forEach((c) => {
        ...
        const isMe =
          currentUserId.value &&
          String(actorId) === String(currentUserId.value);
        rawItems.push({
          id: `comment-${c.id}`,
...
const sendMessage = () => {
  ...
  const optimisticItem = {
    ...
    actor_name:
      profileStore.user?.name || profileStore.user?.first_name || "You",
...
const promptAssignStaff = (staffId) => {
  ...
  const currentStaffId =
    store.detail?.assigned_staff?.id || store.detail?.assigned_staff_id;
...
const priorityClass = (p) =>
  ({
    ...
    urgent:
      "bg-primary-red/30 text-primary-red border-primary-red/40 font-semibold",
