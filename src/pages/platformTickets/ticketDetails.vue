      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          ...
          <div class="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              :class="[
                ...
                store.detail.status !== 'closed' && hasPermission(['ticket.platform_update', 'ticket.update', 'ticket.close', 'ticket.assign'])
                  ? 'cursor-pointer hover:opacity-80'
                  : 'cursor-not-allowed opacity-60',
              ]"
              @click="openStatusDialog"
              :disabled="store.detail.status === 'closed' || !hasPermission(['ticket.platform_update', 'ticket.update', 'ticket.close', 'ticket.assign'])"
            >
              {{ store.detail.status }}
            </button>
        ...
        <div class="flex items-center gap-2 shrink-0">
          <button
            ...
            class="flex items-center gap-1.5 px-3.5 py-2 rounded-xl border border-primary-border bg-card-background hover:bg-background text-secondary-text hover:text-primary-text text-xs font-medium transition-all cursor-pointer shadow-xs disabled:opacity-50"
          ...
          <button
            v-if="store.detail.status !== 'closed' && hasPermission(['ticket.platform_update', 'ticket.update', 'ticket.close', 'ticket.assign'])"
            @click="openStatusDialog"
            class="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-primary hover:bg-primary-hover text-btn-text-primary text-xs font-medium transition-all cursor-pointer shadow-xs"
          >
        ...
        <div class="lg:col-span-1 space-y-4">
          ...
          <div class="bg-card-background border border-primary-border rounded-2xl p-5 shadow-xs space-y-4">
            ...
            <div>
              ...
              <div v-if="store.detail.assigned_staff" class="flex items-center gap-2.5 p-2.5 rounded-xl bg-background border border-primary-border">
              ...
              <p v-else class="text-xs text-secondary-text italic p-2 bg-background rounded-xl border border-primary-border">
            ...
            <div>
              ...
              <div class="flex items-center gap-2.5 p-2.5 rounded-xl bg-background border border-primary-border">
          ...
          <div class="p-4 bg-card-background border-t border-primary-border">
            <form @submit.prevent="sendMessage" class="flex items-end gap-2">
              ...
              <button
                ...
                :disabled="store.actionLoading || store.detail?.status === 'closed'"
