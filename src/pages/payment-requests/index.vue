  ...
  <div class="px-4 pb-8">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 gap-3 mb-5">
      <template v-if="store.loading">
        <div v-for="n in 6" :key="n" class="bg-card-background border border-primary-border rounded-xl p-4 animate-pulse space-y-2">
          <div class="h-3 w-28 bg-background rounded" />
          <div class="h-6 w-16 bg-background rounded" />
        </div>
      </template>
      <template v-else>
        <div v-for="card in summaryCards" :key="card.label" class="bg-card-background border border-primary-border rounded-xl p-4">
          <p class="text-[11px] uppercase tracking-wide text-secondary-text mb-1">{{ card.label }}</p>
          ...
          <tr>
            <td colspan="9" class="py-20 text-center bg-card-background">
              <div class="flex flex-col items-center gap-3">
                ...
                <p class="text-sm font-semibold text-primary-text">No payment requests found</p>
          ...
          >
            ...
            <td class="px-3 py-3.5">
              <div class="flex items-center justify-end gap-1.5">
                ...
                <span v-else class="text-[11px] text-secondary-text">Processed</span>
              </div>
            </td>
          </tr>
        </tbody>
...
const onAccountSearch = (q) => {
  ...
  accountTimer = setTimeout(async () => {
    ...
    const data = res?.data || res || []
    accountOptions.value = (data || []).map(a => ({ label: `#${a.account_number} · ${a.broker_label ?? ''}`.trim(), value: a.account_id }))
  }, 350)
}
...
const summaryCards = computed(() => {
  ...
  return [
    { label: 'Pending Deposits',     value: s.pending_deposits     ?? 0, color: 'text-primary-yellow' },
    { label: 'Approved Deposits',    value: s.approved_deposits    ?? 0, color: 'text-primary-green'  },
    { label: 'Rejected Deposits',    value: s.rejected_deposits    ?? 0, color: 'text-primary-red'    },
    { label: 'Pending Withdrawals',  value: s.pending_withdrawals  ?? 0, color: 'text-primary-yellow' },
    { label: 'Approved Withdrawals', value: s.approved_withdrawals ?? 0, color: 'text-primary-green'  },
    { label: 'Rejected Withdrawals', value: s.rejected_withdrawals ?? 0, color: 'text-primary-red'    },
  ]
})
