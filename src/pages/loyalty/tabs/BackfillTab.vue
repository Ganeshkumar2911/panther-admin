<template>
  <div class="space-y-6">
    ...
    <div class="flex flex-wrap items-center justify-between gap-3">
      <div>
        <h2 class="text-base font-semibold text-primary-text">MT5 History Backfill Jobs</h2>
        <p class="text-xs text-secondary-text">
          Background batch jobs scanning past MT5 trading activity and awarding loyalty points.
        </p>
      </div>
    ...
    >
      ...
      <div class="space-y-1">
        <h3 class="text-sm font-semibold text-primary-text">No Backfill Jobs Found</h3>
        <p class="text-xs text-secondary-text max-w-sm mx-auto">
          {{ filters.trading_account_id ? 'No jobs match this trading account filter.' : 'Queue historical MT5 trade backfills for newly enrolled accounts to credit eligible points.' }}
        </p>
      </div>
      <button
        v-if="hasPermission('loyalty.backfill')"
      ...
      <div class="overflow-x-auto">
        <table class="w-full text-left text-xs">
          <thead class="bg-background/60 text-secondary-text border-b border-primary-border">
            <tr>
              <th class="py-2.5 px-4 text-[11px] font-medium uppercase tracking-wider">Job ID</th>
              <th class="py-2.5 px-4 text-[11px] font-medium uppercase tracking-wider">Trading Account</th>
              <th class="py-2.5 px-4 text-[11px] font-medium uppercase tracking-wider">Date Range</th>
              <th class="py-2.5 px-4 text-[11px] font-medium uppercase tracking-wider">Status</th>
              <th class="py-2.5 px-4 text-[11px] font-medium uppercase tracking-wider">Deals Seen</th>
              <th class="py-2.5 px-4 text-[11px] font-medium uppercase tracking-wider">Awarded</th>
              <th class="py-2.5 px-4 text-[11px] font-medium uppercase tracking-wider">Skipped Exist.</th>
              <th class="py-2.5 px-4 text-[11px] font-medium uppercase tracking-wider">Ineligible</th>
              <th class="py-2.5 px-4 text-[11px] font-medium uppercase tracking-wider">Errors</th>
              <th class="py-2.5 px-4 text-[11px] font-medium uppercase tracking-wider">Started At</th>
          ...
          <tbody class="divide-y divide-primary-border">
            ...
            >
              <td class="py-2.5 px-4 text-secondary-text font-mono">#{{ job.id }}</td>
              <td class="py-2.5 px-4 font-mono font-medium text-primary">#{{ job.trading_account_id }}</td>
              <td class="py-2.5 px-4 text-secondary-text text-[11px] leading-relaxed">
              ...
              <td class="py-2.5 px-4 text-primary-text font-mono">{{ job.deals_seen ?? 0 }}</td>
              <td class="py-2.5 px-4 font-mono font-medium text-primary-green">{{ job.awarded ?? 0 }}</td>
              <td class="py-2.5 px-4 text-secondary-text font-mono">{{ job.skipped_existing ?? 0 }}</td>
              <td class="py-2.5 px-4 text-secondary-text font-mono">{{ job.skipped_ineligible ?? 0 }}</td>
              <td class="py-2.5 px-4 font-mono" :class="job.errors > 0 ? 'text-rose-400 font-medium' : 'text-secondary-text'">
                {{ job.errors ?? 0 }}
              </td>
              <td class="py-2.5 px-4 text-secondary-text">{{ formatDate(job.started_at) }}</td>
