<template>
  <div class="px-4 pb-8">
    ...
    <div class="flex flex-wrap items-start justify-between gap-3 mb-6">
      <div>
        ...
        <p class="text-xs text-secondary-text mt-1">
          Manage banner groups and banners
        </p>
      </div>
      <button
    ...
    >
      <div
        class="w-14 h-14 rounded-2xl bg-card-background border border-primary-border flex items-center justify-center"
      >
      ...
      <div class="text-center">
        ...
        <p class="text-xs text-secondary-text mt-1">
          Create a group to start adding banners.
        </p>
      </div>
    </div>
    ...
    <div v-else class="space-y-4">
      ...
      >
        ...
        <div
          class="p-4 sm:px-5 sm:py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-background/30 cursor-pointer hover:bg-background/50 transition-colors"
          @click="toggleExpand(group.id)"
        >
          <div class="flex items-center gap-3">
            <button
              class="w-6 h-6 flex items-center justify-center rounded-md bg-background border border-primary-border text-secondary-text hover:text-primary transition-colors focus:outline-none shrink-0"
          ...
          <div class="flex items-center gap-3 ml-9 sm:ml-0" @click.stop>
            ...
            >
              <Pencil
                class="w-3.5 h-3.5 text-secondary-text hover:text-primary"
              />
            </button>
            <button
              class="w-7 h-7 flex items-center justify-center rounded-lg hover:bg-background transition-colors"
            ...
            >
              <Loader2
                v-if="store.updateLoading && deletingGroupId === group.id"
                class="w-3.5 h-3.5 text-primary-red animate-spin"
              />
              <Trash2
                v-else
                class="w-3.5 h-3.5 text-secondary-text hover:text-primary-red"
              />
            </button>
            <button
              type="button"
              ...
              :class="
                group.is_active ? 'bg-primary-green' : 'bg-primary-border'
              "
            >
              <span
        ...
        <div
          ...
          class="border-t border-primary-border/50 bg-background/10 p-4 sm:p-5"
        >
          <div
            v-if="!group.banners || group.banners.length === 0"
            class="text-center py-6 text-secondary-text text-xs italic bg-background/30 rounded-xl border border-primary-border/30 dashed"
          >
          ...
          >
            ...
            >
              <!-- Image -->
              <div
                class="relative w-full h-32 bg-background rounded-lg mb-3 overflow-hidden border border-primary-border/50 flex items-center justify-center"
              >
                <img
                  v-if="banner.image_url"
                  :src="banner.image_url"
                  :alt="banner.title"
                  class="w-full h-full object-cover cursor-pointer group-hover/banner:scale-105 transition-transform duration-500"
                  @click="openImagePreview(banner)"
                  @error="handleImageError"
                />
                <div v-else class="text-secondary-text/50">
                ...
                <div class="absolute top-2 left-2 flex gap-1">
                  <span
                    ...
                    :class="
                      banner.is_active
                        ? 'bg-primary-green/80 text-white'
              ...
              <div class="flex items-start justify-between gap-2 mt-auto">
                <div class="min-w-0">
                  ...
                  <p
                    class="text-[10px] text-secondary-text truncate mt-0.5"
                ...
                <div class="flex items-center gap-1 shrink-0">
                  <button
                    class="w-6 h-6 flex items-center justify-center rounded hover:bg-background transition-colors"
                    title="Edit Banner"
                    @click="handleOpenEditBanner(group, banner)"
                  >
                    <Pencil
                      class="w-3 h-3 text-secondary-text hover:text-primary"
                    />
                  </button>
                  <button
                    class="w-6 h-6 flex items-center justify-center rounded hover:bg-background transition-colors"
                    title="Delete Banner"
                    @click="confirmDeleteBanner(group, banner)"
                    :disabled="
                      store.updateLoading && deletingBannerId === banner.id
                    "
                  >
                    <Trash2
                      class="w-3 h-3 text-secondary-text hover:text-primary-red"
                    />
                  </button>
                  <button
                    type="button"
                    title="Toggle Status"
                    :disabled="
                      store.updateLoading && togglingBannerId === banner.id
                    "
                    ...
                    :class="
                      banner.is_active ? 'bg-primary' : 'bg-primary-border'
                    "
                  >
                    <span
                      v-if="
                      ...
                      :class="
                        banner.is_active ? 'translate-x-3' : 'translate-x-0'
                      "
                    />
                  </button>
                </div>
    ...
    <div
      ...
      @click="imagePreview.open = false"
    >
      <div
        class="bg-card-background rounded-2xl border border-primary-border w-full max-w-4xl shadow-2xl overflow-hidden flex flex-col"
        @click.stop
      >
        <!-- Header -->
        <div class="px-5 py-4 border-b border-primary-border flex items-center justify-between bg-background/50">
          <h2 class="text-sm font-semibold text-primary-text">{{ imagePreview.title }}</h2>
          <button
            @click="imagePreview.open = false"
            class="p-1.5 text-secondary-text hover:text-primary-text hover:bg-background rounded-lg transition-colors"
          >
            <X class="w-5 h-5" />
          </button>
        </div>
        ...
        <!-- Image -->
        <div class="p-4 bg-black flex items-center justify-center min-h-[50vh]">
          <img
