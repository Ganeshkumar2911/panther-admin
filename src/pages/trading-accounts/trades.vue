    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
      ...
      <template v-else>
        <div class="bg-card-background border border-primary-border rounded-xl p-4">
          <p class="text-[11px] uppercase tracking-wide text-secondary-text mb-1">Total Lots</p>
          <p class="text-2xl font-medium text-primary-text">{{ formatNum(store.summary.total_lots) }}</p>
        </div>
        <div class="bg-card-background border border-primary-border rounded-xl p-4">
      ...
      <table class="w-full border-collapse">
        <thead>
          <tr class="border-b border-primary-border">
            ...
            <th class="text-left text-[11px] font-medium text-secondary-text uppercase tracking-widest p-3">Entry</th>
            <th class="text-left text-[11px] font-medium text-secondary-text uppercase tracking-widest p-3">Exit</th>
            ...
            <th class="text-right text-[11px] font-medium text-secondary-text uppercase tracking-widest p-3">Created</th>
          </tr>
        </thead>
          ...
          <tr>
            <td colspan="9" class="py-16 text-center">
              <div class="flex flex-col items-center gap-3">
                ...
                <p class="text-xs text-secondary-text">Trades will appear here once executed</p>
              </div>
            </td>
          </tr>
        </tbody>
        ...
        <tbody v-else>
          ...
          >
            <td class="p-3 text-xs font-medium text-primary-text">#{{ trade.trade_id }}</td>

            <td class="p-3 text-xs font-medium text-primary-text">{{ trade.symbol }}</td>
            ...
            <td class="p-3 text-xs text-primary-text tabular-nums">{{ trade.lot.toFixed(2) }}</td>

            <td class="p-3 text-xs text-primary-text tabular-nums">{{ trade.entry_price != null ? trade.entry_price.toFixed(2) : '-' }}</td>
            ...
            <td class="p-3">
              <span class="text-xs tabular-nums font-medium" :class="trade.pnl >= 0 ? 'text-green-700' : 'text-red-700'">
            ...
            <td class="p-3 text-xs text-secondary-text text-right">{{ formatDate(trade.created_at) }}</td>
          </tr>
        </tbody>
      </table>
    </div>
