...
export const useIbWalletStore = defineStore("ibWallet", () => {
  ...
  const fetchIbWallet = (force = false) => {
    ...
    const failureHandler = (err) => {
      ...
      snackbar.show(err?.message || "Failed to fetch IB wallet.", "error");
    };
  ...
  const exportIbWallet = () => {
    isExporting.value = true;

    apiRequest(urls.KEYS.GET, urls.ibLedger.export, {
      params: cleanFilters({
        type: filters.type,
        user_id: filters.user_id,
        from_date: filters.from_date,
      ...
      onFailure: (err) => {
        snackbar.show(err?.message || "Failed to export IB wallet.", "error");
      },
