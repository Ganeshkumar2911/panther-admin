  ...
  >
    ...
    >
      ...
      >
        ...
        <button
          class="w-7 h-7 flex items-center justify-center rounded-lg hover:cursor-pointer hover:bg-background transition-colors"
          @click="emit('close')"
        >
      ...
      <div class="px-6 py-5 space-y-4">
        <div>
          ...
          <span
            class="inline-block text-[11px] font-medium px-3 py-1.5 rounded-full border capitalize"
        ...
        <div>
          <p class="text-xs text-secondary-text mb-1.5">New Priority</p>
          <BaseSelect
      ...
      <div class="px-6 py-4 border-t border-primary-border flex gap-3">
        <button
          class="hover:cursor-pointer flex-1 px-4 py-2.5 rounded-lg text-xs font-medium text-secondary-text border border-primary-border hover:bg-background hover:text-primary-text transition-colors"
          @click="emit('close')"
        >
          Cancel
        </button>
        <button
          :disabled="store.actionLoading || !selected"
          class="hover:cursor-pointer flex-1 px-4 py-2.5 rounded-lg text-xs font-medium text-white bg-primary hover:bg-primary-hover transition-colors flex items-center justify-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
          @click="submit"
        >
          <Loader2
          ...
          />
          <span>{{
            store.actionLoading ? "Updating..." : "Update Status"
          }}</span>
        </button>
      </div>
    </div>
