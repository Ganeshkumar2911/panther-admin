  ...
  <div class="space-y-4">
    ...
    <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-3">
      ...
      >
        ...
        <div class="relative h-9 w-full sm:flex-1">
          <Search
        ...
        <button
          ...
          @click="() => store.fetchFmLeaderboard(true, store.pagination.page, 'dummy')"
        >
          <RefreshCw
            class="h-3.5 w-3.5"
      ...
      <div v-if="layoutMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        ...
        >
          <div>
            ...
            <div class="flex items-start justify-between gap-3 mb-3">
              ...
              <div class="flex flex-col items-end gap-1.5 shrink-0">
                ...
                <div class="h-4 w-20 bg-background rounded-full" />
              </div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-2 mb-3">
              <div class="bg-background/40 border border-primary-border/40 rounded-xl p-2 flex flex-col items-center gap-1.5">
            ...
            <div class="bg-background/40 border border-primary-border/40 rounded-xl p-2.5 mb-3 flex items-center justify-between">
          ...
          <div class="pt-3 mt-2 border-t border-primary-border/60 flex items-center justify-between gap-2">
            <div class="h-7 w-28 bg-background border border-primary-border/60 rounded-lg" />
            <div class="h-7 w-8 bg-background border border-primary-border/60 rounded-lg shrink-0" />
          </div>
        </div>
      </div>
    ...
    <div v-else>
      ...
      >
        ...
        <h3 class="text-base font-semibold text-primary-text mb-1">
          {{ searchQuery ? 'No matching dummy fund managers' : 'No dummy fund managers found' }}
        </h3>
      ...
      >
        <div
          ...
          :key="getFmId(item)"
          class="bg-card-background border border-primary-border hover:border-primary/40 rounded-2xl p-5 transition-all duration-200 flex flex-col justify-between group relative overflow-hidden shadow-xs hover:shadow-md"
        >
          <div>
            ...
            <div class="flex items-start justify-between gap-3 mb-3">
              <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                  ...
                  <p v-if="getUserName(item)" class="text-xs text-secondary-text truncate font-medium">
                    {{ getUserName(item) }}
                  </p>
                </div>
              </div>
              ...
              <div class="flex flex-col items-end gap-1.5 shrink-0">
                ...
                >
                  <span
                    ...
                    :class="getIsActive(item) ? 'bg-primary-green' : 'bg-zinc-400'"
                  />
            ...
            <div
              class="bg-background/80 border border-primary-border/80 rounded-xl px-3 py-2 flex items-center justify-between mb-3.5 shadow-2xs"
            >
            ...
            <div
              class="bg-background/70 border border-primary-border/80 rounded-xl p-3 flex items-center justify-between mb-3 shadow-2xs"
            >
              <div>
                <p class="text-[10px] font-semibold uppercase tracking-wider text-secondary-text">Total Return</p>
                <p class="text-base font-extrabold text-primary-green tracking-tight mt-0.5">
                  +{{ formatMoney(getTotalReturn(item), getCurrency(item)) }}
                </p>
              </div>
              <div class="text-right">
                <p class="text-[10px] font-semibold uppercase tracking-wider text-secondary-text">Win Rate</p>
            ...
            <div class="grid grid-cols-3 gap-2 mb-3">
              <div class="bg-background/40 border border-primary-border/40 rounded-xl p-2 text-center">
                <p class="text-[9px] uppercase font-semibold tracking-wider text-secondary-text mb-0.5">Profit Share</p>
                <p class="text-xs font-bold text-primary-text">{{ formatPercent(getProfitSharing(item)) }}</p>
              </div>
              <div class="bg-background/40 border border-primary-border/40 rounded-xl p-2 text-center">
                <p class="text-[9px] uppercase font-semibold tracking-wider text-secondary-text mb-0.5">Max DD</p>
                <p class="text-xs font-bold text-primary-red">{{ formatPercent(getMaxDrawdown(item)) }}</p>
              </div>
              <div class="bg-background/40 border border-primary-border/40 rounded-xl p-2 text-center">
                <p class="text-[9px] uppercase font-semibold tracking-wider text-secondary-text mb-0.5">Copiers</p>
                <p class="text-xs font-bold text-primary">{{ getCopiers(item) }}</p>
              </div>
            </div>

            <!-- Active Since Footer Info -->
            <div class="bg-background/40 border border-primary-border/40 rounded-xl p-2.5 mb-3 flex items-center justify-between text-xs">
              <span class="text-secondary-text text-[11px]">Active Since</span>
              <span class="font-bold text-primary-text font-mono text-[11px]">{{ formatDate(getActiveSince(item)) }}</span>
            </div>
          </div>

          <!-- Card Action Footer: Full Details & Actions Dropdown Menu -->
          <div
      ...
      <div v-else-if="layoutMode === 'list'" class="space-y-3">
              ...
              >
                ...
                <td class="py-3.5 px-4">
                  <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                      <p class="font-bold text-primary-text text-xs truncate" :title="getLabelName(item)">
                        {{ getLabelName(item) }}
                      </p>
                      <p
                ...
                <td class="py-3.5 px-3">
                  ...
                  <p class="text-[11px] text-secondary-text">
                    Win Rate: <span class="font-semibold text-primary-text">{{ formatPercent(getWinRate(item)) }}</span>
                  </p>
                </td>
                ...
                <!-- Copiers -->
                <td class="py-3.5 px-3">
                  <span class="font-extrabold text-primary text-xs">{{ getCopiers(item) }}</span>
                  <span class="text-[11px] text-secondary-text ml-1">users</span>
                </td>

                <!-- Active Since -->
                <td class="py-3.5 px-3 font-mono text-secondary-text text-[11px]">
        ...
        <div class="md:hidden space-y-3">
          <div
            v-for="item in filteredData"
            :key="getFmId(item)"
            class="bg-card-background border border-primary-border rounded-xl p-4 space-y-3 shadow-xs"
          >
            <div class="flex items-start justify-between gap-2">
              <div class="flex items-center gap-2.5 min-w-0">
                ...
                <div class="min-w-0">
                  <h4 class="font-bold text-primary-text text-sm truncate">{{ getLabelName(item) }}</h4>
                  <p class="text-xs text-secondary-text font-mono truncate">{{ getUserEmail(item) }}</p>
                </div>
              </div>

              <!-- Status Badge -->
              <span
                class="text-[9px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full border shrink-0"
                :class="
                  getIsActive(item)
                    ? 'bg-primary-green/10 text-primary-green border-primary-green/20'
                    : 'bg-background text-secondary-text border-primary-border'
            ...
            <!-- Mobile Metrics Row -->
            <div class="grid grid-cols-2 gap-2 bg-background/50 p-2.5 rounded-lg border border-primary-border/60 text-xs">
              <div>
                <span class="text-[10px] text-secondary-text uppercase">Total Return</span>
                <p class="font-bold text-primary-green">+{{ formatMoney(getTotalReturn(item), getCurrency(item)) }}</p>
              </div>
              <div>
                <span class="text-[10px] text-secondary-text uppercase">Win Rate</span>
                <p class="font-bold text-primary">{{ formatPercent(getWinRate(item)) }}</p>
              </div>
              <div>
                <span class="text-[10px] text-secondary-text uppercase">Profit Share</span>
                <p class="font-bold text-primary-text">{{ formatPercent(getProfitSharing(item)) }}</p>
              </div>
              <div>
                <span class="text-[10px] text-secondary-text uppercase">Copiers</span>
                <p class="font-bold text-primary-text">{{ getCopiers(item) }} users</p>
              </div>
            </div>

            <!-- Mobile Action Row -->
            <div class="flex items-center justify-between pt-2 border-t border-primary-border/60">
              <button
    ...
    <ConfirmationDialog
      ...
      :title="isDummy(itemToToggle) ? 'Switch to Real Fund Manager' : 'Switch to Dummy Fund Manager'"
      :message="`Are you sure you want to switch '${getLabelName(itemToToggle)}' to ${isDummy(itemToToggle) ? 'Real' : 'Dummy'} mode?`"
      :confirm-text="isDummy(itemToToggle) ? 'Switch to Real' : 'Switch to Dummy'"
      cancel-text="Cancel"
      type="warning"
...
const getFmId = (item) =>
  item?.fm_id || item?.dummy_fm?.fm_id || item?.fund_manager?.id || item?.id || "N/A";
...
const getIsActive = (item) => {
  if (item?.fund_manager && typeof item.fund_manager.is_active !== "undefined") {
    return Boolean(item.fund_manager.is_active);
  }
  if (item?.dummy_fm && typeof item.dummy_fm.enabled !== "undefined") {
    return Boolean(item.dummy_fm.enabled);
  }
  if (typeof item?.is_active !== "undefined") return Boolean(item.is_active);
  if (typeof item?.enabled !== "undefined") return Boolean(item.enabled);
  return true;
};
...
const isDummy = (item) => {
  ...
  if (typeof item?.is_dummy !== "undefined") return Boolean(item.is_dummy);
  if (typeof item?.enabled !== "undefined") return Boolean(item.enabled);
  return true;
};
...
const getWinRate = (item) =>
  item?.dummy_fm?.win_rate ?? item?.win_rate ?? 0;
...
const getCurrency = (item) =>
  item?.fund_manager?.broker_currency ||
...
const getRowActions = (item) => {
  const actions = [
    ...
    },
    {
      ...
      icon: isDummy(item) ? Users : Sparkles,
    },
  ];
