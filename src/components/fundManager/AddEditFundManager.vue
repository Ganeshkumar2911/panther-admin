...
watch(
  ...
  { immediate: true },
); // immediate: true to handle initial opening if component is mounted with open=true
...
const validateForm = () => {
  ...
  if (!form.value.password) newErrors.password = "Required";
  ...
  if (Math.abs(fm + broker - 100) > 0.01) {
    newErrors.share_distribution =
      "FM and Broker share must sum to exactly 100%";
  }
...
<template>
  ...
  >
    ...
    >
      <!-- Header -->
      <div
        class="px-5 py-4 border-b border-primary-border flex items-center justify-between flex-shrink-0"
      >
            ...
            <p class="text-secondary-text text-xs mt-0.5">
              {{
                mode === "add"
                  ...
                  : `Update fund manager ID: ${item?.id}`
              }}
            </p>
          </div>
        </div>
      ...
      <!-- Body -->
      <div
        class="px-5 py-5 flex flex-col gap-4 overflow-y-auto flex-1 no-scrollbar"
      >
        <div class="flex flex-col gap-1.5">
          ...
          <input
            ...
            :class="
              errors.label_name
                ? 'border-primary-red'
                : 'border-primary-border focus:border-accent'
            "
          />
          <span v-if="errors.label_name" class="text-xs text-primary-red">{{
            errors.label_name
          }}</span>
        </div>
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <input
            ...
            "
          />
          <span v-if="errors.min_capital" class="text-xs text-primary-red">{{
            errors.min_capital
          }}</span>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="flex flex-col gap-1.5">
            ...
            <input
              ...
              "
            />
            <span v-if="errors.broker_share" class="text-xs text-primary-red">{{
              errors.broker_share
            }}</span>
          </div>
      ...
      <!-- Footer -->
      <div
        class="px-5 py-4 border-t border-primary-border flex gap-2 flex-shrink-0"
      >
        <button
          @click="closeDialog"
