...
const handleProceedLogin = () => {
  ...
  const failureHandler = (err) => {
    ...
    snackbar.show(
      err?.message ||
        err?.error ||
        "Failed to initialize fm dashboard login.",
      "error",
    );
  };

  apiRequest(urls.KEYS.GET, urls.clientList.userDashboard, {
    look_up_key: props.fm?.user?.id || props.fm?.user_id || props.fm?.id,
      ...
      >
        <div class="flex items-center gap-2">
          ...
          <h2 class="text-sm font-bold text-primary-text">
            Login to Fund Manager Dashboard
          </h2>
        </div>
        <button
      ...
      <div class="px-6 py-5 space-y-4 bg-card-background">
        ...
        >
          ...
          <div class="space-y-1">
            ...
            >
              <span>Redirecting to Fund Manager Portal</span>
              <span
            ...
            <p class="text-[11px] text-secondary-text leading-relaxed">
              You will be logged into the fm's dashboard on
          ...
          <div class="space-y-2 text-xs">
            ...
            <div class="flex items-center justify-between">
              ...
              <span class="font-bold text-primary-text">
                {{
                  fm?.user?.name ||
                  `${fm?.user?.first_name || ""} ${fm?.user?.last_name || ""}`.trim() ||
                  fm?.user?.email?.split('@')[0] ||
                  "—"
                }}
              </span>
            </div>
            ...
            <div class="flex items-center justify-between">
              ...
              <span class="font-mono text-primary-text truncate max-w-[200px]">
                {{ fm?.user?.email || fm?.email || "—" }}
              </span>
            </div>

            <!-- Phone -->
            <div v-if="fm?.phone" class="flex items-center justify-between">
              <span
                class="text-secondary-text flex items-center gap-1.5 text-[11px]"
              >
              ...
              <span class="font-medium text-primary-text">
                {{ fm?.user?.phone || fm?.phone }}
              </span>
            </div>

            <!-- Country -->
            <div
              v-if="fm?.user?.country || fm?.country"
            ...
            >
              ...
              >
                <span
                  v-if="getFlagCode(fm?.user?.country || fm?.country)"
                  :class="[
                    'fi',
                    `fi-${getFlagCode(fm?.user?.country || fm?.country)}`,
                    'fis',
                    'w-4 h-3 flex-shrink-0',
                  ]"
                ></span>
                <span>{{ cleanCountryLabel(fm?.user?.country || fm?.country) || "—" }}</span>
              </span>
            </div>
          </div>
        </div>
