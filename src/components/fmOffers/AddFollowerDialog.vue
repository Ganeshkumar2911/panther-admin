<template>
  ...
  >
    ...
    >
      <!-- Dialog Header -->
      <div class="flex items-center justify-between px-6 py-4 border-b border-primary-border shrink-0 bg-background/50">
      ...
      <div class="p-6 space-y-5 text-xs overflow-y-auto flex-1">
        ...
        >
          ...
          <div class="flex-1">
            ...
            <p class="text-[11px] opacity-90 mt-0.5 leading-relaxed">{{ errorMessage }}</p>
          </div>
          <button
            type="button"
        ...
        <div class="space-y-1.5">
          <div class="flex items-center justify-between">
            <label class="font-bold text-primary-text flex items-center gap-1.5">
              <span>1. Select Client</span>
              <span class="text-primary-red">*</span>
            </label>
            <span v-if="selectedClient" class="text-[10px] text-secondary-text font-mono">
          ...
          >
            <div class="flex items-center gap-2.5 min-w-0">
              <div class="w-7 h-7 rounded-lg bg-primary/10 text-primary flex items-center justify-center font-bold text-xs shrink-0">
                {{ (selectedClient.name || selectedClient.label || 'U')[0].toUpperCase() }}
              </div>
              <div class="min-w-0">
                <p class="font-bold text-primary-text truncate">{{ selectedClient.name || selectedClient.label }}</p>
                <p class="text-[11px] text-secondary-text font-mono truncate">{{ selectedClient.email || 'No email' }}</p>
              </div>
            </div>
            <span class="px-2 py-0.5 rounded text-[10px] font-mono bg-background border border-primary-border text-secondary-text shrink-0">
        ...
        <div class="space-y-2">
          <label class="font-bold text-primary-text flex items-center justify-between">
          ...
          <div class="grid grid-cols-2 gap-2.5">
            ...
            >
              <div class="flex items-center justify-between w-full mb-1">
                ...
                <span
                  ...
                  :class="form.mode === 'copy_trading' ? 'bg-primary' : 'bg-transparent border border-secondary-text'"
                />
              </div>
              <p class="text-[10px] opacity-80 leading-relaxed">
                Deposits into FM coverage account; trades replicated proportionally.
              </p>
            </button>
            ...
            >
              <div class="flex items-center justify-between w-full mb-1">
                ...
                <span
                  class="w-2 h-2 rounded-full"
                  :class="form.mode === 'real' ? 'bg-primary' : 'bg-transparent border border-secondary-text'"
                />
              </div>
              <p class="text-[10px] opacity-80 leading-relaxed">
                Independent real trading account participating directly in strategy.
              </p>
            </button>
          </div>

          <p v-if="modeNotice" class="text-[10px] text-amber-500 flex items-center gap-1 mt-1">
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            ...
            <span v-if="accountsLoaded" class="text-[10px] font-mono text-secondary-text">
          ...
          >
            ...
            <p class="text-[11px] text-secondary-text/80 mt-0.5">
              Eligible accounts matching the FM's currency and criteria will load automatically.
            </p>
          </div>
          ...
          >
            ...
            <ul class="text-[11px] opacity-90 list-disc list-inside space-y-0.5 pl-1">
          ...
          <div v-else class="space-y-2">
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div
                ...
                :class="form.trading_account_id === acc.trading_account_id
                  ? 'bg-primary/10 border-primary text-primary shadow-xs'
              ...
              >
                <div class="flex items-center justify-between mb-1">
                  <div class="flex items-center gap-1.5">
                    <span class="font-mono font-bold text-xs">#{{ acc.account_number }}</span>
                    <span
                  ...
                  >
                    <Check v-if="form.trading_account_id === acc.trading_account_id" class="w-3 h-3" />
                  </div>
                </div>

                <div class="flex items-baseline justify-between mt-2 pt-1 border-t border-primary-border/40">
                ...
                <div class="flex items-center justify-between text-[10px] text-secondary-text mt-0.5">
        ...
        <div class="space-y-2">
          <div class="flex items-center justify-between">
            <label class="font-bold text-primary-text flex items-center gap-1.5">
              ...
              <span class="text-[10px] font-normal text-secondary-text">(Optional)</span>
            </label>
            <span class="text-[10px] text-secondary-text">
          ...
          >
            <div class="flex items-center justify-between">
              <div>
                <p class="font-bold text-primary-text">{{ selectedOffer.name }}</p>
                <p v-if="selectedOffer.offer_code" class="text-[10px] font-mono text-primary font-bold">
                  Code: {{ selectedOffer.offer_code }}
                </p>
              </div>
              <span
                class="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full border"
                :class="selectedOffer.is_active
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 pt-1 border-t border-primary-border/60">
              <div>
                <span class="text-[9px] uppercase text-secondary-text block font-semibold">Perf Fee</span>
                <span class="font-bold font-mono text-primary-text">{{ selectedOffer.performance_fee ?? 0 }}%</span>
              </div>
              <div>
                <span class="text-[9px] uppercase text-secondary-text block font-semibold">Mgmt Fee</span>
                <span class="font-bold font-mono text-primary-text">{{ selectedOffer.management_fee ?? 0 }}%</span>
              </div>
              <div>
                <span class="text-[9px] uppercase text-secondary-text block font-semibold">Reg Fee</span>
                <span class="font-bold font-mono text-primary-text">
                  {{ formatCurrency(selectedOffer.registration_fee, activeCurrency) }}
                </span>
              </div>
              <div>
                <span class="text-[9px] uppercase text-secondary-text block font-semibold">Min Balance</span>
                <span class="font-bold font-mono text-primary">
                  {{ formatCurrency(selectedOffer.minimum_balance, activeCurrency) }}
                </span>
              </div>
            </div>
          </div>
          ...
          >
            ...
            <div>
              ...
              <p class="mt-0.5 leading-relaxed">
                Selected account balance ({{ formatCurrency(selectedAccount?.balance, selectedAccount?.broker_currency) }}) is lower than the offer's minimum balance requirement ({{ formatCurrency(selectedOffer?.minimum_balance, activeCurrency) }}). The join will be rejected by the backend.
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="border border-primary-border rounded-xl p-3.5 bg-background/40 space-y-2.5">
          <div class="flex items-center justify-between">
            <span class="font-bold text-xs text-primary-text">5. Lot Settings (Optional)</span>
            <span class="text-[10px] text-secondary-text">Default lot rules applied if omitted</span>
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
              <label class="text-[11px] font-semibold text-secondary-text block mb-1">Lot Sizing Type</label>
              <BaseSelect
            ...
            <div>
              <label class="text-[11px] font-semibold text-secondary-text block mb-1">Lot Sizing Value</label>
              <input
          ...
          <p class="text-[10px] text-secondary-text">
            • <strong>Fixed:</strong> execute trades with exact lot volume. • <strong>Multiplier:</strong> scale proportionally to master trade volume.
          </p>
        </div>

        <!-- 6. Confirmation Notice (Required UX Copy) -->
        <div class="p-3.5 rounded-xl bg-primary-yellow/10 border border-primary-yellow/25 text-primary-yellow text-xs space-y-1">
          ...
          <p class="text-[11px] opacity-90 leading-relaxed">
            This is <strong>not</strong> a free admin override. The operation executes under standard client join rules:
          </p>
          <ul class="text-[11px] opacity-90 list-disc list-inside space-y-0.5 pl-1">
            <li>Any configured registration fee will be charged to the client account.</li>
            <li>Minimum capital / balance limits are strictly enforced.</li>
            <li>Copy-trading accounts deposit funds into the Fund Manager coverage account.</li>
          </ul>
        </div>
      </div>

      <!-- Action Footer -->
      <div class="flex items-center justify-between gap-3 px-6 py-4 border-t border-primary-border shrink-0 bg-background/50">
        <button
          type="button"
        ...
        >
          ...
          <span>{{ isSubmitting ? 'Adding Follower...' : 'Confirm & Add Follower' }}</span>
        </button>
      </div>
    </div>
  </div>
...
import {
  UserPlus, X, AlertCircle, AlertTriangle, Wallet, Check,
  Loader2, ShieldAlert
...
const selectedClient = computed(() => {
  ...
  return clientOptions.value.find(c => String(c.value) === String(form.user_id)) || null
})

const selectedAccount = computed(() => {
  if (!form.trading_account_id) return null
  return availableAccounts.value.find(a => a.trading_account_id === form.trading_account_id) || null
})

const selectedOffer = computed(() => {
  if (!form.offer_id) return null
  return offers.value.find(o => String(o.id) === String(form.offer_id)) || null
})

const offerSelectOptions = computed(() => {
  const list = [
    { label: 'No Offer (FM Default Fees)', value: null }
  ]
  ...
  clientSearchTimer = setTimeout(async () => {
    try {
      ...
      clientOptions.value = (results || []).map(c => ({
        label: c.name || c.label || c.email || `#${c.value || c.id}`,
        name: c.name || c.label,
        value: c.value ?? c.id,
        email: c.email || '',
      }))
...
const loadInitialClients = async () => {
  isSearchingClients.value = true
  try {
    const results = await clientLedgerStore.fetchAllClients('')
    clientOptions.value = (results || []).map(c => ({
      label: c.name || c.label || c.email || `#${c.value || c.id}`,
      name: c.name || c.label,
      value: c.value ?? c.id,
...
const fetchAvailableAccounts = () => {
  ...
  const endpoint = urls.fm.availableJoiners || '/fund_managers/followers/available-joiners'
  ...
  apiRequest(urls.KEYS.GET, endpoint, {
    ...
    onFailure: (err) => {
      ...
      if (err?.follower_account_type !== undefined || err?.message?.toLowerCase?.().includes('trading type')) {
        modeNotice.value = err?.message || `Mode "${form.mode}" is not permitted by this Fund Manager.`
      } else {
        errorMessage.value = err?.message || 'Failed to load eligible accounts.'
      }
    },
  })
}
...
const fetchOffers = () => {
  ...
  apiRequest(urls.KEYS.GET, endpoint, {
    ...
    onSuccess: (res) => {
      ...
      offers.value = list.sort((a, b) => (b.is_active ? 1 : 0) - (a.is_active ? 1 : 0))

      // Pre-select defaultOfferId if provided and exists
      if (props.defaultOfferId && list.some(o => String(o.id) === String(props.defaultOfferId))) {
