...
function getStaffName(l) {
  if (l.assigned_staff) {
    return `${l.assigned_staff.first_name || ''} ${l.assigned_staff.last_name || ''}`.trim() || l.assigned_staff.email
  }
    ...
    <Transition name="slide-right">
      ...
      >
          ...
          <div class="flex items-center gap-3 min-w-0">
            ...
            <div class="min-w-0">
              <div class="flex items-center gap-2">
                ...
                <span class="text-[10px] font-bold px-2 py-0.5 rounded-full border uppercase shrink-0 bg-primary/10 text-primary border-primary/20">
                  {{ getStageName(activeLead) }}
                </span>
              </div>
              <p class="text-xs text-secondary-text truncate mt-0.5">
                Code: {{ activeLead.lead_code || `L-${activeLead.id}` }} · Created {{ formatDate(activeLead.created_at || activeLead.createdAt) }}
              </p>
            </div>
          </div>
        ...
        <div class="flex-1 overflow-y-auto p-6 space-y-6 no-scrollbar">
          ...
          <div v-if="leadStore.detailLoading" class="py-12 flex flex-col items-center justify-center gap-2 text-secondary-text">
          ...
          <template v-else>
            ...
            <div class="bg-background/80 border border-primary-border/80 rounded-2xl p-4 space-y-3">
              ...
              <div class="grid grid-cols-2 gap-3 text-xs">
                <div>
                  ...
                  <span class="text-primary-text font-medium">{{ getLeadName(activeLead) }}</span>
                </div>
                ...
                <div>
                  ...
                  <span class="text-primary-text font-medium truncate block" :title="activeLead.email">
                    {{ activeLead.email || '-' }}
                  </span>
                </div>
                ...
                <div>
                  ...
                  <span class="text-primary-text font-medium">{{ activeLead.source || 'Website' }}</span>
                </div>

                <div>
                  <span class="text-secondary-text text-[11px] block mb-1">Assigned Staff</span>
                  <BaseSelect
                    :model-value="activeLead.assigned_staff?.id || activeLead.assigned_staff_id || null"
                ...
                <div>
                  ...
                  <span class="text-primary-text font-medium">{{ activeLead.priority || 'Medium' }}</span>
                </div>

                <div>
                  <span class="text-secondary-text text-[11px] block">Current Stage</span>
                  <span class="text-primary font-semibold">{{ getStageName(activeLead) }}</span>
                </div>
              </div>

              <!-- Remarks / Note -->
              <div v-if="activeLead.remarks" class="pt-2 border-t border-primary-border/40 text-xs">
                <span class="text-secondary-text text-[11px] block mb-1">Remarks</span>
                <p class="text-primary-text bg-background p-2.5 rounded-lg border border-primary-border/60">
                  {{ activeLead.remarks }}
                </p>
              </div>
            </div>
            ...
            <div
              v-if="activeLead.current_followup_date || activeLead.followUp?.date"
            ...
            >
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="text-[11px] font-semibold uppercase tracking-wider text-secondary-text">Follow-up Schedule</p>
                  <p class="text-xs font-bold text-primary-text mt-0.5">
                    {{ formatDate(activeLead.current_followup_date) }}
                  </p>
                  <p v-if="activeLead.current_followup_note" class="text-[11px] text-secondary-text mt-0.5">
            ...
            <div class="space-y-3">
              <div class="flex items-center justify-between">
                ...
                <span class="text-[10px] text-secondary-text font-mono">
                  {{ leadStore.leadHistory.length }} Events Logged
                </span>
              </div>

              <div v-if="leadStore.historyLoading" class="py-6 flex items-center justify-center text-xs text-secondary-text gap-2">
                <Loader2 class="w-4 h-4 animate-spin text-primary" />
                <span>Loading history...</span>
              </div>

              <div v-else-if="leadStore.leadHistory.length === 0" class="py-6 text-center text-xs text-secondary-text border border-dashed border-primary-border rounded-xl">
                No stage transitions recorded yet.
              </div>

              <div v-else class="relative pl-6 space-y-4 border-l-2 border-primary-border ml-2">
                <div
                  v-for="item in leadStore.leadHistory"
                  :key="item.id"
                  ...
                  <div class="bg-background border border-primary-border/60 rounded-xl p-3 hover:border-primary/40 transition-colors">
                    <div class="flex items-center justify-between mb-1">
                      <h4 class="text-xs font-bold text-primary-text">
                        {{ item.from_stage_name || 'Start' }} → {{ item.to_stage_name }}
                      </h4>
                      <span class="text-[10px] text-secondary-text font-mono">{{ formatDate(item.created_at) }}</span>
                    </div>
                    <p class="text-[11px] text-secondary-text leading-relaxed">
                      Changed by <span class="text-primary font-medium">{{ item.changed_by_name || 'Staff' }}</span> ({{ item.change_source || 'Manual' }})
                      <span v-if="item.remarks" class="block text-primary-text mt-0.5 font-sans italic">
                        "{{ item.remarks }}"
                      </span>
                    </p>
                  </div>
