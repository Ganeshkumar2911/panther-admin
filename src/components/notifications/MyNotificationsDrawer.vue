  ...
  <Teleport to="body">
    <div>
      <Transition name="backdrop">
        <div v-if="open" class="fixed inset-0 z-[100] bg-black/50" @click="emit('close')" />
      </Transition>
      ...
      <Transition name="drawer">
        ...
        >
          <!-- Header -->
          <div class="px-5 py-4 border-b border-primary-border flex items-center justify-between shrink-0">
            <div class="flex items-center gap-3">
              ...
              <div>
                <div class="flex items-center gap-2">
                  <h2 class="text-sm font-semibold text-primary-text">Notifications</h2>
                  <span v-if="store.unreadCount > 0" class="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-primary text-black min-w-[18px] text-center">
                    {{ store.unreadCount }}
                  </span>
                </div>
                <p class="text-[11px] text-secondary-text mt-0.5">Your alerts and updates</p>
              </div>
            </div>
            ...
            <div class="flex items-center gap-1.5">
              ...
              >
                <RotateCw class="w-3.5 h-3.5" :class="{ 'animate-spin': store.myLoading }" />
              </button>
              <button
              ...
              <button class="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-background text-secondary-text hover:text-primary-text transition-colors" @click="emit('close')">
                <X class="w-4 h-4" />
              </button>
            </div>
          </div>
          ...
          <!-- Filters -->
          <div class="px-5 py-3 border-b border-primary-border shrink-0 space-y-2.5">
            <div class="relative">
            ...
            <div class="grid grid-cols-3 gap-2">
              <select v-model="selectedReadStatus" class="px-2 py-1.5 text-xs rounded-lg bg-background border border-primary-border text-primary-text outline-none focus:border-primary transition-colors">
                <option :value="null">All Status</option>
                <option value="unread">Unread</option>
                <option value="read">Read</option>
              </select>
              <select v-model="selectedPriority" class="px-2 py-1.5 text-xs rounded-lg bg-background border border-primary-border text-primary-text outline-none focus:border-primary transition-colors">
                <option :value="null">All Priority</option>
                <option value="HIGH">High</option>
                <option value="MEDIUM">Medium</option>
                <option value="LOW">Low</option>
              </select>
              <select v-model="selectedType" class="px-2 py-1.5 text-xs rounded-lg bg-background border border-primary-border text-primary-text outline-none focus:border-primary transition-colors">
          ...
          <!-- List -->
          <div class="flex-1 overflow-y-auto mx-2 my-4 border border-primary-border rounded-xl divide-y divide-primary-border">
            <!-- Skeleton -->
            <div v-if="store.myLoading" class="p-4 space-y-3">
              <div v-for="n in 4" :key="n" class="h-24 bg-card-background border border-primary-border rounded-xl animate-pulse" />
            </div>

            <!-- Empty -->
            <div v-else-if="filteredMyNotifications.length === 0" class="flex flex-col items-center gap-3 py-20">
              <div class="w-12 h-12 rounded-full bg-background border border-primary-border flex items-center justify-center">
            ...
            <div v-else class="divide-y divide-primary-border">
              ...
              >
                <!-- Unread bar -->
                <div v-if="!item.is_read" class="absolute left-0 top-0 bottom-0 w-0.5 bg-primary" />

                <!-- Top row: type + priority + date -->
                <div class="flex items-center justify-between gap-2 mb-2 pl-3">
                  <div class="flex items-center gap-1.5">
                    <span class="text-[10px] font-semibold px-1.5 py-0.5 rounded border uppercase" :class="typeClass(item.type)">
                      {{ item.type }}
                    </span>
                    <span class="text-[10px] font-semibold px-1.5 py-0.5 rounded border uppercase" :class="priorityClass(item.priority)">
                      {{ item.priority }}
                    </span>
                  </div>
                  <span class="text-[11px] text-secondary-text shrink-0">{{ formatDate(item.created_at) }}</span>
                </div>

                <!-- Title + message -->
                <div class="pl-3 mb-3">
                  <p class="text-xs font-semibold text-primary-text mb-0.5">{{ item.title }}</p>
                  <p class="text-[11px] text-secondary-text leading-relaxed">{{ item.message }}</p>
                </div>
                ...
                <div class="pl-3 flex items-center justify-between">
                  ...
                  <div class="flex items-center gap-1.5">
                    <template v-if="item.is_read">
                      ...
                      <span class="text-[10px] text-secondary-text">Read {{ formatDate(item.read_at) }}</span>
                    </template>
                    <template v-else>
                      <Check class="w-3.5 h-3.5 text-secondary-text" />
                      <span class="text-[10px] text-secondary-text">Unread</span>
                    </template>
                  </div>
                  ...
                  <div class="flex items-center gap-2">
                    <a
                      ...
                      target="_blank"
                      class="flex items-center gap-1 text-[11px] text-primary hover:text-primary-hover transition-colors font-medium"
                    >
            ...
            <!-- See More Button -->
            <div
              v-if="store.myPagination.has_more || store.myPagination.page < store.myPagination.total_pages"
            ...
            >
              ...
              >
                <Loader2 v-if="store.myMoreLoading" class="w-3.5 h-3.5 animate-spin" />
                <ChevronDown v-else class="w-3.5 h-3.5" />
                <span>{{ store.myMoreLoading ? 'Loading more...' : 'See More Notifications' }}</span>
              </button>
            </div>
          </div>
        </div>
...
import { Bell, CheckCheck, Check, RotateCw, Search, ExternalLink, X, ChevronDown, Loader2 } from 'lucide-vue-next'
...
watch(() => props.open, (val) => { if (val) store.fetchMyNotifications() })
...
const formatDate = (v) => v
  ? new Date(v).toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })
  : '—'
</script>
...
.drawer-enter-active, .drawer-leave-active { transition: transform 0.25s cubic-bezier(0.16,1,0.3,1); }
