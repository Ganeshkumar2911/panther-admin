<template>
  ...
  >
    <div
      class="bg-card-background rounded-2xl border border-primary-border w-full max-w-2xl max-h-[90vh] flex flex-col"
      @click.stop
    >
      <div
      ...
      >
        <div>
          <h2 class="text-sm font-semibold text-primary-text">
            {{ template?.subject ?? "Email Preview" }}
          </h2>
      ...
      <div class="flex-1 overflow-y-auto p-6">
        <div
          class="bg-white rounded-xl border border-primary-border p-6 min-h-48"
        >
          <div
            v-if="template?.body"
          ...
          />
          <p v-else class="text-secondary-text text-sm text-center py-8">
        ...
        <div v-if="template?.available_variables?.length" class="mt-4">
          <p
            class="text-[11px] text-secondary-text uppercase tracking-wide mb-2"
          >
            Variables
          </p>
