<template>
  <div class="space-y-4">
    <!-- Info Banner -->
    <div class="p-3.5 rounded-xl bg-primary-red/10 border border-primary-red/20 flex items-start gap-3">
      <Key class="w-4 h-4 text-primary-red shrink-0 mt-0.5" />
      <div class="text-xs">
        <p class="font-medium text-primary-text">Active Custom Overrides</p>
        <p class="text-secondary-text text-[11px] mt-0.5">
          Select custom permission overrides to <strong>remove</strong> them for this user.
        </p>
      </div>
    </div>
    ...
    >
      <div class="w-12 h-12 rounded-full bg-background border border-primary-border flex items-center justify-center mb-3">
        ...
        <Sliders class="w-5 h-5 text-secondary-text" />
      </div>
      <p class="text-xs font-semibold text-primary-text">
        {{ searchQuery ? 'No matching custom overrides found' : 'No custom overrides active' }}
      </p>
      <p class="text-[11px] text-secondary-text mt-1">
        {{ searchQuery ? 'Try adjusting your search criteria' : 'This user currently has no user-specific custom permission overrides.' }}
      </p>
    </div>
    ...
    >
      ...
      <div class="px-4 py-3 bg-card-background/50 border-b border-primary-border flex items-center justify-between">
        <div class="flex items-center gap-2">
          <Layers class="w-4 h-4 text-primary-red" />
          ...
          <span class="text-[10px] px-2 py-0.5 rounded-full bg-background text-secondary-text border border-primary-border font-medium">
            {{ getSelectedCount(perms) }} / {{ perms.length }} Selected to Remove
          </span>
        </div>
        ...
        >
          ...
          <div
            ...
            :class="isResourceFullySelected(perms) ? 'bg-primary-red border-primary-red' : 'bg-background border-primary-border'"
          >
      ...
      <div class="p-3 grid grid-cols-1 sm:grid-cols-2 gap-2">
        <div
          ...
          :class="selectedIds.includes(getPermId(perm))
            ? 'bg-primary-red/10 border-primary-red/40 text-primary-text'
        ...
        >
          ...
          <div class="flex items-center gap-1.5 shrink-0">
            <span v-if="selectedIds.includes(getPermId(perm))" class="text-[9px] font-bold px-1.5 py-0.5 rounded bg-primary-red/20 text-primary-red uppercase">
              Will Remove
            </span>
            <div
              class="w-4 h-4 rounded border flex items-center justify-center transition-colors"
              :class="selectedIds.includes(getPermId(perm)) ? 'bg-primary-red border-primary-red' : 'bg-background border-primary-border'"
            >
...
import { Search, Key, Sliders, Layers, Check } from 'lucide-vue-next'
