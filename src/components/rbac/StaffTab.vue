<template>
  <div>
    ...
    <!-- Table -->
    <div class="border border-primary-border rounded-2xl overflow-hidden">
      <table class="w-full border-collapse">
        <thead class="border-b border-primary-border bg-card-background">
          <tr>
            <th class="text-left text-[11px] font-medium text-secondary-text uppercase tracking-widest px-4 py-3">Name</th>
            ...
            <th class="text-right text-[11px] font-medium text-secondary-text uppercase tracking-widest px-4 py-3">Actions</th>
          </tr>
        </thead>

        <!-- Skeleton -->
        <tbody v-if="staffStore.loading">
          <tr v-for="n in 5" :key="n" class="border-b border-primary-border bg-card-background animate-pulse">
            ...
            <td class="px-4 py-3.5 flex justify-end"><div class="h-7 w-16 bg-background rounded-lg" /></td>
          </tr>
        </tbody>
        ...
        <tbody v-else-if="staffStore.records.length === 0">
          <tr>
            <td colspan="5" class="py-20 text-center bg-card-background">
        ...
        <tbody v-else>
          ...
          >
            <td class="px-4 py-3.5">
              <div class="flex items-center gap-2.5">
                <div class="w-7 h-7 rounded-full bg-primary flex items-center justify-center text-[10px] font-bold text-black shrink-0">
            ...
            <td class="px-4 py-3.5">
              <select
                ...
                class="px-2 py-1.5 text-xs rounded-lg bg-background border border-primary-border text-primary-text outline-none focus:border-primary transition-colors disabled:opacity-50"
                @change="staffStore.updateStaffRole(staff.id, Number($event.target.value))"
              >
